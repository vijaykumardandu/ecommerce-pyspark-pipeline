...
All data is synthetic, generated by `src/generate_data.py` using the Faker library.
The generator intentionally injects data quality problems — null amounts, duplicate
order IDs, future timestamps, mixed currencies — to make the cleaning and validation
steps meaningful. Running the generator twice will produce different data each time
because UUIDs and timestamps are random, but the distribution of issues stays the same.
